...
# segment tree index find

ll quary(int u,int b,int e,int val)
{
   if(b==e)
    {      
        return b;
     ...
     int   mid=(b+e)/2;
      if(tr[2*u]>=val)return quary(2*u,b,mid,val);
       return quary((2*u)+1,mid+1,e,val-tr[2*u]);
}
...
21.Longest Palindromic Substring O(N) Manachers Algorithm
int main() {
  int i, j, k, n, m;
  string s;
  cin >> s;
...
39
*/
#Using Manacher Algorithm prefix +suffix maximum palinedrome 
int main() {
   int t;
    cin>>t;
    while(t--)
...
//nth non square number
 ans(n)=n+floor(0.5+(double)sqrt(n));

//Basic counting
i.  (nc0)^2 +(nc1)^2+(nc2)^2+.....+(ncn)^2=(2nCn)
ii. (nc0)+((n+1)C1)+((n+2)c2)+..+((n+k)ck)=(n+k+1)Ck
iii. 0.(nc0)+1.(nc1)+2.(nc2)+...+n(ncn)=n*(2^(n-1))
//Star & Bars
i.  x1+x2+x3+...xk=N 
  ans=(N-1)c(k-1

 (All time we consider xi>=1)
 suppose x1+x2=3
  x1>=-5,x2>=2
  convert x1'=x1+6,x2'=x2-1

ii. x1+x2+x3.. <=N and xi>=1
  x1+x2+x3<=10
  we write x1+x2+x3+c<=10
  c=0,1,2,3,4,5,6,7;
       n-k  n-c-1
  ans= E       C     //E summation
       c=0      k-1
...
iv.n lenth er sequence a atleast 0 or 1 or 2 once occur
  total=3^n
  badsequence=n*((2^n)-1);
  good=3^n -n*((2^n) -1)
...
//Series 
i.   1+2+3+4.........n=(n*(n+1))/2
ii.  1^2+2^2+3^2..+n^2=(n*(n+1)*(n+2))/6
iii. 1^3+2^3+3^3..+n^3=((n*(n+1))/2)^2
iv.  4+7+10+......+n
     nth=a+(n-1)*d
     Sn=(n*(1st+nth))/2
     or Sn=(n*(2a+(n-1)*d))/2
...
vi.  2+4+6+8.....+n=n*(n+1)
vii. 1+3+5+7+....+n=n^2
viii for(k=1 to n)sum+=(k*(k+1))
     formula=(n*(n+1)*(n+2))/3
ix.  for(k=1 to n)sum+=1/(k*(k+1))  
     formula=n/(n+1)
x.   for(k=1 to n)sum+=(k*(k+1)*(k+2))  
     formula=(n*(n+1)*(n+2)*(n+3))/4
xi.  for(k=1 to n)sum+=1/(k*(k+1)*(k+2))  
     formula=(n*(n+3))/(4*(n+1)*(n+2))
XII. 2^2 +4^2+6^2+...+98^2
     nth=a+(n-1)*d
     98=2*(n-1)*2
     n=49
     for(1 to 49)=(2n)^2=4*n^2
    formula=4*((n*(n+1)*(n+2))/6)
